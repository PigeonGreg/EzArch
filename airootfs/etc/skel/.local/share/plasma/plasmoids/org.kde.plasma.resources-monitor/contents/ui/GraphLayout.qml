    ...
    Repeater {
        ...
        delegate: Loader {
            ...
            width: {
                if (!item?.visible)
                    return 0;
                if (modelData.sizes[0] !== -1)
                    return modelData.sizes[0];
                if (root.isVertical)
                    return root.parentWidth;
            ...
            height: {
                if (!item?.visible)
                    return 0;
                if (modelData.sizes[1] !== -1)
                    return modelData.sizes[1];

                return root.isVertical ? root.parentWidth : root.parentHeight;
            }
